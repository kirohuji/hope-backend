...
processes.register = Meteor.bindEnvironment(function ({
  instanceId,
  source,
  isResume = false,
}) {
  ...
  if (!registeredDoc) {
    return BpmnProcessCollection.insert({
      instanceId,
      source,
      isResume,
      state: "started",
...
processes.updateState = Meteor.bindEnvironment(function updateState(
  instanceId,
  state
) {
  check(instanceId, String);
...
function runExtensions({ mergedExtensions, name, instance, options }) {
  ...
  extensionValues.forEach((extension) => {
    if (extension[name]) {
      extension[name].call(null, instance, options);
    }
  });
}
...
function cleanOptions(options) {
  ...
  Object.keys(tmp).forEach((key) => {
    if (validExecuteOptions.indexOf(key) === -1) {
      delete tmp[key];
    }
  });
  return tmp;
...
function createListeners(callbackFct, opts, arr) {
  ...
  if (Object.prototype.hasOwnProperty.call(options, "target")) {
    ...
    if (!(options.target instanceof EventEmitter)) {
      throw new Error("expected target to be an EventEmitter");
    }
  }
...
Bpmn.Engine.resume = function (state, options, callback) {
  ...
  engineRef = originalResume.call(
    this,
    state,
    cleanOptions(_options),
    (error, engine) => {
      engineRef = engine;
