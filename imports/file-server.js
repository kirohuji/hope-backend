...
async function userProcess(users, profile) {
  return Promise.all(users.map(async function (user) {
    ...
    let username = pinyin(mapUser.username, { pattern: 'first', toneType: 'none' }).replace(/\s/g, '');
    ...
    if (!_id) {
      ...
      ProfilesCollection.update({ _id: _id }, {
        $set: {
          realName: mapUser.username,
          email: `${usernameWithAge}@lourd.online`,
          ..._.pick(mapUser, ["displayName", "phoneNumber", "age", "gender", "photoUrl", "country", "isPublic", "state", "city", "address", "about", "baptized"]),
          scope: profile?.scope
        }
      })
      return true
...
if (Meteor.isServer) {
  ...
  Picker.route('/storage/upload', function (params, req, res, next) {
    ...
    res.setHeader("Access-Control-Allow-Headers", "X-Auth-Token,Content-Type,Accept");
    res.setHeader('Access-Control-Allow-Methods', 'GET,POST,PUT,DELETE,OPTIONS');
    if (req.file !== undefined && req.file.mimetype && params.query.authToken.length) {
        ...
        _fs.stat(req.file.path, function (_statError, _statData) {
          ...
          _fs.readFile(req.file.path, function (_readError, _readData) {
            if (_readError) {
              ...
              Storage.write(_readData, _addFileMeta, function (_uploadError, _uploadData) {
                if (_uploadError) {
  ...
  Picker.route('/storage/excel', async function (params, req, res, next) {
    ...
    res.setHeader("Access-Control-Allow-Headers", "X-Auth-Token,Content-Type,Accept");
    res.setHeader('Access-Control-Allow-Methods', 'GET,POST,PUT,DELETE,OPTIONS');
    if (req.file !== undefined && isExcel(req.file.originalname) && params.query.authToken.length) {
      const hashedToken = Accounts._hashLoginToken(params.query.authToken);
      const user = Meteor.users.findOne({ 'services.resume.loginTokens.hashedToken': hashedToken });
      if (user) {
        try {
